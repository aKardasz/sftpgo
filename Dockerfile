...
RUN set -xe && \
    ...
    go build $(if [ -n "${FEATURES}" ]; then echo "-tags ${FEATURES}"; fi) -trimpath -ldflags "-s -w -X github.com/aKardasz/sftpgo/v2/internal/version.commit=${COMMIT_SHA} -X github.com/aKardasz/sftpgo/v2/internal/version.date=`date -u +%FT%TZ`" -v -o sftpgo
